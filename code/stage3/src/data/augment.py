...
def farthest_seed_indices(points: np.ndarray, num_seeds: int) -> np.ndarray:
    n_points = points.shape[0]
    num_seeds = min(int(num_seeds), n_points)
    if num_seeds <= 0:
        return np.empty((0,), dtype=np.int64)

    seed_idx = np.empty((num_seeds,), dtype=np.int64)
    seed_idx[0] = np.random.randint(n_points)

    diff = points - points[seed_idx[0]]
    min_dist2 = np.sum(diff * diff, axis=1)
    min_dist2[seed_idx[0]] = -1.0

    for i in range(1, num_seeds):
        seed_idx[i] = int(np.argmax(min_dist2))
        diff = points - points[seed_idx[i]]
        dist2 = np.sum(diff * diff, axis=1)
        min_dist2 = np.minimum(min_dist2, dist2)
        min_dist2[seed_idx[: i + 1]] = -1.0
...
class AugmentPatch(Augment):
    ...
    def apply(self, asset: Asset, **kwargs):
        ...
        seed_idx = farthest_seed_indices(pc_noisy, self.num_patches)  # (P,)
