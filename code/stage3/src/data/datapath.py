...
_OBJ_MESH_CACHE: Dict[str, tuple[np.ndarray, np.ndarray]] = {}
...
class ObjLazyAsset(LazyAsset):
    ...
    def load(self) -> 'Asset':
        cached = _OBJ_MESH_CACHE.get(self.path)
        if cached is None:
            import trimesh
            ...
            _OBJ_MESH_CACHE[self.path] = cached
...
@dataclass
class PlyLazyAsset(LazyAsset):
    # load a binary/ASCII .ply point cloud file as an asset (for predict mode)
    def load(self) -> 'Asset':
        with open(self.path, "rb") as f:
            ...
            while True:
                line = f.readline()
                if line == b"":
                    raise ValueError(f"invalid ply header: {self.path}")
            ...
            if not header_lines or header_lines[0] != "ply":
                raise ValueError(f"not a ply file: {self.path}")
            ...
            if vertex_count is None:
                raise ValueError(f"ply file has no vertex element: {self.path}")
            ...
            xyz = ["x", "y", "z"]
            if fmt == "ascii":
                pc = np.loadtxt(f, max_rows=vertex_count, usecols=[0, 1, 2], dtype=np.float64)
            elif fmt == "binary_little_endian":
                dtype = np.dtype([(name, prop_types[kind]) for kind, name in properties])
                vertices = np.fromfile(f, dtype=dtype, count=vertex_count)
                pc = np.stack([vertices[name] for name in xyz], axis=1).astype(np.float64)
            else:
                raise ValueError(f"unsupported ply format {fmt}: {self.path}")
